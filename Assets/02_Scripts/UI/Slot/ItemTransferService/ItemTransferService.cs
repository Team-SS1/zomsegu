...
public static class ItemTransferService
{
    ...
    // 슬롯에서 외부로 드롭
    public static bool TryDropOutside(SlotRef from, WorldDropSpawner worldDropSpawner) // 인스턴스형 아이템
    {
        return DropTransferService.TryDropOutside(from, -1, worldDropSpawner);
    }
    public static bool TryDropOutside(SlotRef from, int amount, WorldDropSpawner worldDropSpawner) // 스택형 아이템
    {
        return DropTransferService.TryDropOutside(from, amount, worldDropSpawner);
    }
