...
public class MonsterInitialBehavior : MonoBehaviour
{
    ...
    private IEnumerator Initial_Patrol()
    {
        ...
        else
        {
            ...
            if (patrolType == PatrolType.Vertical)
            {
                patrolTarget = (patrolTarget.y > patrolStart.y)
                    ? patrolStart
                    : patrolStart + new Vector3(0, patrolDistance, 0);
            }
            else if (patrolType == PatrolType.Horizontal)
            {
                patrolTarget = (patrolTarget.x > patrolStart.x)
                    ? patrolStart
                    : patrolStart + new Vector3(patrolDistance, 0, 0);
            }
        }
